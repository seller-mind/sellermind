...
export function BatchClient() {
  ...
  const optimizeListings = async (data: BatchData, isRegenerate = false) => {
    // F-UX-01 fix: pre-flight email check to match other 4 tools.
    // Without this, empty-email users saw a misleading "already used 3 free uses"
    // error from the server-side LIMIT_EXCEEDED branch when their real problem was
    // "email not set". This aligns UX with listing/reply/holiday/review.
